...
void main()
{
    ...
    float sunPct = circle(interp_UV, 1.0, 0.0);
    
    // sun haze
    //float sunHazePct = circle(interp_UV, 0.57, 0.2) * 0.35;
    //fragColor.rgb = (vec3(0.909, 0.167, 0.596) * sunHazePct);
    ...
    float lineSize = mix(0.4, -0.2, interp_UV.y);
    ...
    if(sunColor * sunPct == vec3(0.0, 0.0, 0.0)){
    	discard;
    }
    
    // Add all together
    //fragColor.rgb = ((1.0 - sunPct) * fragColor.rgb) + (sunColor * sunPct);
    fragColor.rgb = sunColor * sunPct;
}