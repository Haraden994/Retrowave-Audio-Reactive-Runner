...
void main()
{
	...
	vec3 startingColor = specularColor;
	vec3 actualSColor = specularColor;
	float blinkSpeed = 20.0;
	float pct = abs(sin(time*blinkSpeed));
	vec3 blinkColor;
	if(blink == 1){
		blinkColor = vec3(0.0, 1.0, 0.0); //green blink for speed up
		actualSColor = mix(startingColor, blinkColor, pct);
	}
	else if(blink == -1){
		blinkColor = vec3(1.0, 0.0, 0.0); //red blink for speed up
		actualSColor = mix(startingColor, blinkColor, pct);
	}
	else
		actualSColor = startingColor;
    ...
    vec3 finalColor = (lambert + specular) * actualSColor * NdotL;
